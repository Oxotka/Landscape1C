...
`app/survey2026.js` хранит июльский этап (247 участников),
`app/survey2026-current.js` — опубликованный снимок текущего опроса.
Витрина и карточки объединяют инструменты с приоритетом текущих данных. Для локального просмотра основной волны
есть `./survey-live.command` (см. ниже).
...
Текущую волну можно посмотреть локально одной командой из корня проекта:

```bash
./survey-live.command
```

Команда передает журнал с сервера в агрегатор потоком, не сохраняет сырые
ответы на Mac и открывает локальную витрину. Файл агрегатов
`app/survey2026-live.js` не попадает в Git и публичную сборку.
...
Получение и публикация разделены: `survey-live.command` обновляет только
игнорируемый `app/survey2026-live.js`. После просмотра выбранный снимок отдельно
копируется в `app/survey2026-current.js`, проверяется и коммитится для публикации.
