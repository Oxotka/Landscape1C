...
**Architecture:** Общая бизнес-логика (`bot.js`: `onMessage`/`onCallback`) остаётся одна на обе платформы — `bot-max.js` (новый вебхук-процесс) собирает из событий MAX тот же telegram-подобный конверт, который эти функции уже понимают, и вызывает их напрямую. Транспорт расширяется тремя именованными функциями (`editCard`/`answerCallback`/`setupCommands`), убирающими из `bot.js` последние сырые вызовы `api()` с telegram-специфичными именами методов. `lib/store.js` становится platform-aware (раздельные файлы состояния, безопасная соль uid) без изменения формулы для телеграма — в `answers.jsonl` уже боевые данные волны.
...
**Interfaces:**
- Consumes: `setupTestEnv` (Task 2), `installFakeTelegram` (Task 2), `onMessage`/`onCallback` из `bot.js` (пока не экспортированы явно — до Task 6 они module-level функции, доступны через `require("../bot.js")` только после того, как Task 6 добавит `module.exports`; **этот тест первым делом проверяет, что для его написания уже нужен экспорt — см. Step 1 ниже, где это явно оговорено как временный шаг**.

Важное уточнение по порядку задач: `bot.js` до Task 6 **не экспортирует** `onMessage`/`onCallback` (`module.exports` в файле нет вообще). Чтобы регрессионные тесты могли зафиксировать поведение **до** рефакторинга (что и есть их смысл — страховка на Task 6), нужно сначала выполнить минимальную, отдельно коммитящуюся правку — добавить один `module.exports = { onMessage, onCallback };` в конец `bot.js`, ничего больше не трогая. Это первый шаг этого таска.

- [ ] **Step 1: Добавить экспорт в `bot.js` (единственная правка до рефакторинга)**
...
- [ ] **Step 2: Прогнать бота вручную, убедиться что он всё ещё стартует**
...
## Task 6: `bot.js` — убрать сырые telegram-вызовы, разрешить require без старта polling

**Files:**
- Modify: `bot/bot.js:13` (импорт), `bot/bot.js:85-103` (`ensureAnchor`), `bot/bot.js:640-656` (ответ на "a"), `bot/bot.js:541` (`answerCallback`), `bot/bot.js:770-839` (обёртка `require.main`), `bot/bot.js:789-807` (замена на `setupCommands`)
...
- [ ] **Step 1: Заменить импорт транспорта**
...
- [ ] **Step 6: Обернуть long polling в `require.main === module`**
...
и в самом конце файла (было `})();`). Заменить весь блок «── Long polling ──» так, чтобы он выполнялся только при прямом запуске (`node bot.js`), а не при `require("./bot")` из `bot-max.js` — иначе второй процесс тоже запустит telegram-polling тем же токеном (два polling на один токен дерутся за апдейты, README уже предупреждает об этом для двух запущенных копий самого бота):

```js
// ── Long polling ──
// require.main === module: блок ниже выполняется только при "node bot.js"
// напрямую, не при require("./bot") из bot-max.js (иначе MAX-процесс
// тоже запустит telegram-polling тем же токеном)
...
- [ ] **Step 7: Прогнать полный регрессионный набор — это и есть проверка «ничего не сломали»**

Run: `node --test bot/test/`
Expected: `# pass 6`, `# fail 0`. Если что-то упало — не подгонять тест под новое поведение, разбираться в diff по шагам 1–6 (например: `editCard` при `isPhoto=false` должен ставить именно `params.text`, не `params.caption`).

- [ ] **Step 8: Ручная регресс-проверка на боевом коде (без реальной отправки)**
...
- [ ] **Step 9: Форматирование и коммит**

```bash
npx prettier --write bot/bot.js
git add bot/bot.js
git commit -m "Бот: убрать сырые telegram-вызовы (editCard/answerCallback/setupCommands), require.main для polling"
```

- [ ] **Step 10: Задеплоить на VPS и проверить телеграм-бота живьём**

Это единственный шаг таска, который трогает боевой сервер — делать после того, как Step 7 и 8 зелёные локально.

```bash
ssh root@194.87.62.61 'cd /opt/landscape1c && git pull && systemctl restart stateof1c && sleep 2 && systemctl status stateof1c --no-pager -l | head -15'
```

Expected: `active (running)`, без ошибок в первых строчках лога. Дальше вручную (не автоматизируется) — написать боту `/start` в телеграме, пройти пару карточек, поставить и снять паузу: карточки, якорь и кнопки должны вести себя как до правки.
